...
directory = 'C:\\Users\\allan\PycharmProjects\Thesis\Guldkorpus/transcriptions/org/working/'
    ...
    for filename in filenames:
        ...
        for row in d:
            if row[None][0].strip() == "w":  # rows containing word will be kept
                ...
                # lemma = row[None][7].strip()
                strings.append(lemma)
    return '\n'.join(strings)


def doc_representation(filenames):
    documents = []
    for filename in filenames:
        strings = []
        working_file = directory + filename
        ...
        mo = re.search("Transcription\n.*\n(\n|$)", data, re.S)
        table = mo.group(0)
        doc = csv.DictReader(table.splitlines(), delimiter="|")
        for row in doc:
            if row[None][0].strip() == "w":  # rows containing word will be kept
                lemma = row[None][7].strip()  # 7th column is the facsimile level transcription - take those as lemmas
                strings.append(lemma)
        documents.append(strings)
...
# prep images and targets
paths = image_paths()
years, docids = get_targets(paths)
binned_years = bin_years(years)
# prep text documents
doc_names = [doc_filenames(i) for i in docids]

missing_transcriptions = ['17.org', '30.org', '37.org', '471.org', '472.org', '477.org', '479.org', '480.org', '483.org', '484.org', '485.org', '489.org']
count = 0
for docname in missing_transcriptions:
    binned_years.pop(doc_names.index(docname)-count)
    paths.pop(doc_names.index(docname)-count)
    swt_paths.pop(doc_names.index(docname)-count)
    docids.pop(doc_names.index(docname)-count)
    count += 1
for i in missing_transcriptions:
    doc_names.remove(i)
...
X = doc_representation(doc_names)
Xx = [" ".join(x) for x in X]
# words = [item for sublist in Xx for item in sublist]
# words = [''.join(x) for x in words]
# words = ''.join(words)


def construct_pipeline():
    pipe = Pipeline([('count', CountVectorizer(analyzer='char', ngram_range=(1, 5), lowercase=False, binary=False)), ('tfid', TfidfTransformer(sublinear_tf=True))]).fit(Xx)
    return pipe


pipe = construct_pipeline()
# pipe = alternate_pipeline()
# pipe = third_pipeline()
new = pipe.fit_transform(Xx)


def extract_features(swt_paths):
    # edge_features = []
    swt_features = []
    ...
    features = np.array(swt_features)
    return features


# image_features = extract_features(edge_paths, swt_paths)
image_features = extract_features(swt_paths)
...
def arrange(X, binned_years):
    X = np.array(X)
    binned_years = np.array(binned_years)
    indices1250 = np.where(binned_years == 1250)
    indices1300 = np.where(binned_years == 1300)
    indices1350 = np.where(binned_years == 1350)
    indices1400 = np.where(binned_years == 1400)
    indices1450 = np.where(binned_years == 1450)
    indices1500 = np.where(binned_years == 1500)
    indices1550 = np.where(binned_years == 1550)
...
def custom_train_test_split(features, targets, array_of_indices):
    ...
    choices = np.random.choice(possibilities, 84)  # Picking 20% of the documents at random
    ...
    return train_indices, test_indices, train_targets, test_targets, choices, possibilities
...
# Xtrain, Xtest, ytrain, ytest, choices, possibilities = custom_train_test_split(0, np.array(binned_years), doc_indices)

# X_test_nlp = new[choices]
# X_train_nlp = np.arange(0, 421, 1)
# X_train_nlp = np.delete(X_train_nlp, choices)
# X_train_nlp = new[X_train_nlp]

# X_test_img = X[choices]
# X_train_img = np.arange(0, 422, 1)
# X_train_img = np.delete(X_train_img, choices)
# X_train_img = X[X_train_img]

del(Xx)
del(directory)
del(this_file)
del(missing_transcriptions)
del(paths)
del(pipe)
del(swt_paths)
del(doc_names)
...
# X_train_img2, y_train_img2 = arrange(X_train_img, ytrain)
# y_train_img2 = np.asarray(y_train_img2)
# X_test_img2, y_test_img2 = arrange(X_test_img, ytest)
# y_test_img2 = np.asarray(y_test_img2)
...
# chunk_indices = np.arange(0, 422).tolist()
# chunks = partition(chunk_indices, 5)

quant_x, quant_y = arrange(X, binned_years)


def chonkers():
    bin = np.array(binned_years)
    indices1250 = np.where(bin == 1250)
    indices1300 = np.where(bin == 1300)
    indices1350 = np.where(bin == 1350)
    indices1400 = np.where(bin == 1400)
    indices1450 = np.where(bin == 1450)
    indices1500 = np.where(bin == 1500)
    indices1550 = np.where(bin == 1550)
    fold1250 = np.array(partition(indices1250[0], 5))
    fold1300 = np.array(partition(indices1300[0], 5))
    fold1350 = np.array(partition(indices1350[0], 5))
    fold1400 = np.array(partition(indices1400[0], 5))
    fold1450 = np.array(partition(indices1450[0], 5))
    fold1500 = np.array(partition(indices1500[0], 5))
    fold1550 = np.array(partition(indices1550[0], 5))
    all_folds = np.vstack((fold1250.T, fold1300.T, fold1350.T, fold1400.T, fold1450.T, fold1500.T, fold1550.T)).T
    fold1 = np.hstack((all_folds[0]))
    fold2 = np.hstack((all_folds[1]))
    fold3 = np.hstack((all_folds[2]))
    fold4 = np.hstack((all_folds[3]))
    fold5 = np.hstack((all_folds[4]))
    folds = np.array((fold1, fold2, fold3, fold4, fold5))
    return folds


stratifiedfolds = chonkers()  # Random stratified 20% of all the data in each fold, no overlaps.
...
def first_layer():
    years = np.array(binned_years)
    blepy = np.array(quant_y)
    nlp_predictions = []
    img_predictions = []
    actual_years = []
    for fold in stratifiedfolds:
        test_ids = np.arange(0, 422, 1)
        test_ids = test_ids[fold]
        train_ids = np.arange(0, 422, 1)
        train_ids = np.delete(train_ids, fold)
        ytest = years[fold]
        actual_years.append(ytest)
        ytrain = np.delete(years, fold)
        test_nlp = new[fold]
        train_nlp = np.arange(0, 422, 1)
        train_nlp = np.delete(train_nlp, fold)
        train_nlp = new[train_nlp]
        clf = LinearSVC(penalty='l2', loss='squared_hinge', dual=True, fit_intercept=False, class_weight='balanced',
                        multi_class='ovr', max_iter=1000)
        clf.fit(train_nlp, ytrain)
        nlp_pred = clf.predict(test_nlp)
        nlp_predictions.append(nlp_pred)

        train_indices = np.isin(doc_indices, train_ids)
        train_indices = np.nonzero(train_indices)  # Indices in doc_indices where test image patches are

        train_features_img = quant_x[train_indices]
        train_targets_img = blepy[train_indices]

        clf2 = RandomForestClassifier(random_state=42)
        clf2.fit(train_features_img, train_targets_img)

        pred = []
        for vector in test_ids:
            test_indices = np.isin(doc_indices, vector)
            test_indices = np.nonzero(test_indices)
            test_vector = quant_x[test_indices]
            vector_predictions = clf2.predict(test_vector)
            pred.append(vector_predictions)

        final_predictions = []
        for i in pred:
            candidate = np.bincount(i).argmax()
            final_predictions.append(candidate)
        img_predictions.append(final_predictions)
    return np.array(nlp_predictions), np.array(img_predictions), np.array(actual_years)


text_predictions, image_predictions, dates = first_layer()

X1 = np.concatenate((text_predictions))
X2 = np.concatenate((image_predictions))
Y = np.concatenate((dates))
stacked_X = np.column_stack((X1, X2))
...
# clf = LogisticRegression(fit_intercept=False, n_jobs=-1, random_state=42)
cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
# scores = cross_val_score(clf, stacked_X, Y, scoring='f1_weighted', cv=cv, n_jobs=-1)
scores = cross_val_score(clf, stacked_X, Y, scoring='balanced_accuracy', cv=cv, n_jobs=-1)
...
# train_features_img = X_train_img[np.concatenate(Xtrain)]
# train_targets = ytrain[np.concatenate(ytrain)]
...
clf.fit(X_train_img, train_targets)
...
doc_names.index('17.org')
binned_years.pop(doc_names.index('17.org'))
paths.pop(doc_names.index('17.org'))
doc_names.index('30.org')
binned_years.pop(doc_names.index('30.org'))
paths.pop(doc_names.index('30.org'))
doc_names.index('37.org')
binned_years.pop(doc_names.index('37.org'))
doc_names.index('471.org')
binned_years.pop(doc_names.index('471.org'))
doc_names.index('472.org')
binned_years.pop(doc_names.index('472.org'))
doc_names.index('477.org')
binned_years.pop(doc_names.index('477.org'))
doc_names.index('479.org')
binned_years.pop(doc_names.index('479.org'))
doc_names.index('480.org')
binned_years.pop(doc_names.index('480.org'))
doc_names.index('483.org')
binned_years.pop(doc_names.index('483.org'))
doc_names.index('484.org')
binned_years.pop(doc_names.index('484.org'))
doc_names.index('485.org')
binned_years.pop(doc_names.index('485.org'))
doc_names.index('489.org')
binned_years.pop(doc_names.index('489.org'))
'''